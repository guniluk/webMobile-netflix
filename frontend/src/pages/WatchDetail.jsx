...
export default function WatchDetail() {
  ...
  return (
    <div className="min-h-screen bg-[#141414] text-white flex flex-col pb-20">
      ...
      <main className="flex-1 pt-24 max-w-6xl mx-auto w-full px-4">
        {/* Back Button */}
        <button
          onClick={() => navigate(-1)}
        ...
        <div className="w-full aspect-video bg-zinc-950 rounded-lg overflow-hidden border border-zinc-800 shadow-2xl relative mb-10 group">
          {hasTrailer ? (
            <iframe
              src={`https://www.youtube.com/embed/${trailers[currentTrailerIdx].key}?autoplay=0`}
              title="Trailer video"
            ...
            >
              <p className="text-xl md:text-2xl font-bold text-zinc-300 mb-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-12 mb-16">
          ...
          <div className="flex justify-center">
            <img
              ...
              className="w-full max-w-[320px] rounded-lg shadow-xl border border-zinc-800"
            />
          </div>

          {/* Details */}
          <div className="md:col-span-2 flex flex-col gap-4">
            <h1 className="text-3xl md:text-5xl font-black">{details.title || details.name}</h1>
            ...
            <div className="flex flex-wrap items-center gap-4 text-sm text-zinc-400">
              ...
              {details.adult && (
                <span className="bg-red-600 text-white text-xs font-extrabold px-1.5 py-0.5 rounded">
                  18+
                </span>
              )}
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              {details.genres?.map((genre) => (
                <span
                  key={genre.id}
                  className="badge badge-outline border-zinc-700 text-zinc-300 py-2.5 px-3"
                >
                  {genre.name}
                </span>
              ))}
            ...
            {/* Overview / Storyline */}
            <div>
              <h3 className="font-bold text-lg mb-2 text-zinc-200">Overview</h3>
        ...
        {similar.length > 0 && (
          <div className="relative group/similar border-t border-zinc-900 pt-10">
              ...
              {similar.map((item) => {
                ...
                return (
                  <Link
                    key={item.id}
                    to={`/watch/${item.id}`}
                    className="min-w-45 sm:min-w-55 md:min-w-65 relative rounded-md overflow-hidden transition-all duration-300 hover:scale-105 shadow-md group"
                  >
                    <img
                      src={imgUrl}
